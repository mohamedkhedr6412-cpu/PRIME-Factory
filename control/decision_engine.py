"""
PRIME-Factory Decision & Explainability Engine v4.0
Maps multi-modal evidence to operational decisions and generates live Decision Trace cards.
"""
import config
...
class DecisionEngine:
    ...
    def evaluate_decision(
        ...
        """
        توليد القرار التشغيلي وتفكيك الأدلة السببية الكاملة (Explainable AI - Decision Trace)
        """
        # قواعد اتخاذ القرار
        if current_state in ["MAINTENANCE", "RECOVERY"]:
            ...
            consequence_of_inaction = "Impending line breakdown within ~15 min (60-min downtime, $350/hr penalty)"
        elif current_state == "PREDICTIVE_ALERT" or (health_index <= config.HI_THRESHOLDS["MONITOR"] and is_confirmed_anomaly):
            ...
            consequence_of_inaction = "Bearing failure will progress to Critical, causing 4x downtime and scrap"
        ...
        return {
            ...
            "rul_display": f"{rul_minutes} min" if (rul_minutes is not None and rul_minutes > 0) else "RUL Not Established",
