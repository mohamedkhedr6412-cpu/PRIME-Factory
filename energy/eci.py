"""
PRIME-Factory Context-Aware Energy Condition Indicator (ECI) v3.0
Fixes C3 Bug: Computes expected power using Commanded Reference Speed to ensure a true healthy baseline.
"""
import config

def get_context_expected_power(machine_id: str, product_key: str) -> float:
    """
    حساب القدرة الطبيعية المتوقعة بناءً على السرعة التصميمية للمنتج (Commanded Speed)
    وليس السرعة اللحظية المقاسة التي تتأثر بالتدهور الميكانيكي.
    """
    machine_nominal = config.MACHINES[machine_id]["nominal_kw"]
    prod = config.PRODUCTS[product_key]
    
    # السرعة التصميمية المطلوبة للمنتج
...
def calculate_eci(actual_power_kw: float, machine_id: str, product_key: str, epsilon: float = 1e-3) -> float:
    """
    حساب الانحراف اللحظي للطاقة:
    ECI = (P_actual - P_expected(Commanded)) / P_expected
    """
