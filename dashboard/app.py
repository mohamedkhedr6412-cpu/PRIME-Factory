"""
PRIME-Factory Interactive Industrial Control & Decision Center v6.1 (Final)
...
st.set_page_config(
    page_title="PRIME-Factory | Control & Decision Center v6.1",
    layout="wide",
    page_icon="🏭"
)
...
if "simulation_run" not in st.session_state:
    st.session_state.simulation_run = None
if "results_cache" not in st.session_state:
    st.session_state.results_cache = None


st.title("🏭 PRIME-Factory: Industrial Control & Decision Center v6.1")
...
if j_step == 1:
    ...
    enable_chaos = False
    apply_dr = False
    demo_step_desc = "Healthy Multi-Product Baseline"
elif j_step == 2:
    ...
    enable_chaos = False
    apply_dr = False
    demo_step_desc = "Fault Injection & Detection"
elif j_step == 3:
    ...
    demo_step_desc = "Manual Configuration"
...
if st.session_state.results_cache is None:
    with st.spinner("Running simulation..."):
        ...
        st.session_state.results_cache = sim_result
else:
    sim_result = st.session_state.results_cache
...
with t_live:
    ...
    if "vibration_rms" in df_target_view.columns and config.MACHINES[selected_machine]["has_vibration"]:
        fig_cond.add_trace(
            go.Scatter(x=df_target_view["timestep"], y=df_target_view["vibration_rms"],
                       name="Vibration (g RMS)", line=dict(color="#1f77b4")),
            row=1, col=1
        )

    if "temperature_c" in df_target_view.columns:
        fig_cond.add_trace(
            go.Scatter(x=df_target_view["timestep"], y=df_target_view["temperature_c"],
                       name="Temperature (°C)", line=dict(color="#ff7f0e")),
            row=1, col=1
        )

    if "power_kw" in df_target_view.columns or "active_power_kw" in df_target_view.columns:
        power_col = "power_kw" if "power_kw" in df_target_view.columns else "active_power_kw"
        fig_cond.add_trace(
            go.Scatter(x=df_target_view["timestep"], y=df_target_view[power_col],
                       name="Power (kW)", line=dict(color="#2ca02c")),
            row=1, col=1
        )

    if "health_index" in df_target_view.columns:
        fig_cond.add_trace(
            go.Scatter(x=df_target_view["timestep"], y=df_target_view["health_index"],
                       name="Health Index (HI)", line=dict(color="#00cc96", width=2.5)),
            row=2, col=1
        )
...
# ============================================================
st.divider()
st.caption("🏭 PRIME-Factory v6.1 | Team MSA | RoboDam 2026")