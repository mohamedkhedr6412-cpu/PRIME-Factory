"""
PRIME-Factory Interactive Industrial Control & Decision Center v4.0 (Master Edition)
Includes: Machine Selector (M1-M5), Multi-Fault Injection, True Context Residuals, Time Scrubber, Live What-If, and XAI Trace.
"""
import sys
import os
...
st.set_page_config(page_title="PRIME-Factory | Control & Decision Center", layout="wide", page_icon="🏭")
...
st.title("🏭 PRIME-Factory: Industrial Control & Decision Center v4.0")
...
max_deg = st.sidebar.slider("Degradation Severity (%):", min_value=10, max_value=85, value=45) / 100.0
...
time_scrubber = st.sidebar.slider("Playback Horizon (Minutes):", min_value=30, max_value=config.TOTAL_TIMESTEPS, value=config.TOTAL_TIMESTEPS, step=10)

# ==========================================
# Master Simulation Pipeline (True Context Residuals)
# ==========================================
@st.cache_data
def run_master_pipeline(mode, prod_key, target_m, f_type, fault_enabled, start_t, max_d, chaos_enabled, dr_enabled, manual_pdm):
    ...
    if mode == "Fixed Product Regime":
        ...
        event_log.add_event(0, "RECIPE_SCHEDULED", "INFO", "ALL", "Dynamic Multi-Product Switching Recipe (A -> B -> C) loaded.")
    ...
    if fault_enabled:
        if "Bearing" in f_type:
            ...
            event_log.add_event(start_t, "BEARING_DEGRADATION", "WARNING", target_m, f"Mechanical bearing wear inception at t={start_t} min.")
        elif "Friction" in f_type:
            profile = generate_friction_profile(config.TOTAL_TIMESTEPS, start_time=start_t, max_friction=max_d)
            event_log.add_event(start_t, "MECHANICAL_FRICTION", "WARNING", target_m, f"Mechanical friction / lubrication breakdown at t={start_t} min.")
        else:
            profile = generate_electrical_profile(config.TOTAL_TIMESTEPS, start_time=start_t, max_severity=max_d)
            event_log.add_event(start_t, "ELECTRICAL_ANOMALY", "WARNING", target_m, f"Stator winding / electrical insulation drift at t={start_t} min.")
    ...
    if chaos_enabled:
        ...
        df.loc[df["machine_id"] == target_m, "vibration_rms"] = inject_sensor_noise_spikes(vib_vals, spike_probability=0.04)
    ...
    # 1. التدريب على الوضع السليم
    healthy_training_df = df[df["degradation"] == 0.0].copy()
    
    # حساب الفروق المعيارية الحقيقية (True Context Residuals)
    ...
    ai_model_raw = PRIMEIsolationForest(contamination=0.02)
    ai_model_raw.fit(healthy_training_df)
    df["raw_ai_score"] = ai_model_raw.predict_anomaly_score(df)

    # نموذج السياق الحقيقي: تدريب على الفروق المعيارية لإلغاء تأثير تبديل المنتجات
    ...
    context_features = healthy_training_df[["power_residual", "vibration_rms", "temperature_c"]].copy()
    from sklearn.ensemble import IsolationForest
    c_model = IsolationForest(n_estimators=100, contamination=0.02, random_state=config.RANDOM_SEED)
    c_model.fit(context_features)
    ...
    for idx, row in target_df.iterrows():
        ...
        hi_list.append(current_hi)
        pers_list.append(p_info["persistence_ratio"])
        conf_list.append(p_info["is_confirmed_anomaly"])
        
        r_val, r_str = estimate_rolling_rul(hi_list, window_size=15)
        rul_val_list.append(r_val if r_val is not None else -1)
        rul_str_list.append(r_str)

        # التدخل التفاعلي
        in_maint = False
        if manual_pdm and t >= (start_t + 60) and t < (start_t + 75):
            in_maint = True
            if t == (start_t + 60):
                maint_timestep = t
                event_log.add_event(t, "MAINTENANCE_EXECUTED", "INFO", target_m, "Operator executed 15-minute targeted predictive maintenance.", recommended_action="Allow 15 min repair")

        st_name = state_machine.update_state(
            degradation=row["degradation"] if not (manual_pdm and t >= start_t + 75) else 0.0,
            health_index=current_hi if not (manual_pdm and t >= start_t + 75) else 95.0,
            is_confirmed_anomaly=p_info["is_confirmed_anomaly"],
            in_maintenance=in_maint
        )
        states_list.append(st_name)

        if st_name == "PREDICTIVE_ALERT" and alert_timestep is None:
            alert_timestep = t
            event_log.add_event(t, "PREDICTIVE_ALERT", "PREDICTIVE", target_m, f"Health Index dropped to {current_hi:.1f} (RUL: {r_str}). Targeted repair recommended.")
    ...
    # Alert Latching
    active_worst_state = "NORMAL"
    if not manual_pdm and fault_enabled:
        if "CRITICAL" in states_list:
            active_worst_state = "CRITICAL"
        elif "PREDICTIVE_ALERT" in states_list:
            active_worst_state = "PREDICTIVE_ALERT"
        elif "WARNING" in states_list:
            active_worst_state = "WARNING"
    else:
        active_worst_state = states_list[-1]

    latest_decision = DecisionEngine.evaluate_decision(
        machine_id=target_m,
        current_state=active_worst_state,
        health_index=hi_list[-1] if not manual_pdm else 95.0,
        rul_minutes=rul_val_list[-1] if not manual_pdm else None,
    ...
    return df, target_df, event_log, latest_decision, rul_str_list[-1], alert_timestep, maint_timestep, active_worst_state
...
fin_metrics = calculate_financial_and_esg_impact(
    total_energy, 
    downtime_minutes=15 if st.session_state.manual_maintenance_triggered else (60 if display_state == "FAILED" else 0), 
...
kpi3.metric(f"{selected_machine} Health Index", f"{latest_hi:.1f} / 100", delta=f"{latest_hi - 100:.1f}", delta_color="inverse")
kpi4.metric("Estimated RUL", latest_rul_str)
kpi5.metric("Shift Operational Cost", f"${fin_metrics['energy_cost_usd']:.1f}", delta=f"{fin_metrics['carbon_kg']:.1f} kg CO2", delta_color="off")

# Alert Banner
if display_state in ["PREDICTIVE_ALERT", "CRITICAL"]:
...
elif display_state in ["MAINTENANCE", "RECOVERY"]:
...
# Operational Tabs
# ==========================================
t_live, t_dec, t_whatif, t_events, t_bench, t_ablation = st.tabs([
...
with t_live:
    fig_cond = make_subplots(
        ...
        subplot_titles=(f"Physical Telemetry ({selected_machine} - {config.MACHINES[selected_machine]['name']})", "PRIME Composite Health Index (HI) & Event Timeline")
    )
    if config.MACHINES[selected_machine]["has_vibration"]:
        fig_cond.add_trace(go.Scatter(x=df_target_view["timestep"], y=df_target_view["vibration_rms"], name="Vibration RMS (g)", line=dict(color="#1f77b4", width=2)), row=1, col=1)
    fig_cond.add_trace(go.Scatter(x=df_target_view["timestep"], y=df_target_view["temperature_c"], name="Temperature (°C)", line=dict(color="#ff7f0e", dash="dot")), row=1, col=1)
    fig_cond.add_trace(go.Scatter(x=df_target_view["timestep"], y=df_target_view["power_kw"], name="Power (kW)", line=dict(color="#d62728")), row=1, col=1)
    
    fig_cond.add_trace(go.Scatter(x=df_target_view["timestep"], y=df_target_view["health_index"], name="Health Index (HI)", line=dict(color="#2ca02c", width=3)), row=2, col=1)
    ...
    if inject_fault and fault_start <= time_scrubber:
        fig_cond.add_vline(x=fault_start, line_dash="dot", line_color="#ffbb00", annotation_text="Fault Start", row=2, col=1)
    if alert_t is not None and alert_t <= time_scrubber:
        fig_cond.add_vline(x=alert_t, line_dash="dash", line_color="#fd7e14", annotation_text="Predictive Alert", row=2, col=1)
    if maint_t is not None and maint_t <= time_scrubber:
        fig_cond.add_vline(x=maint_t, line_dash="solid", line_color="#20c997", annotation_text="PdM Executed", row=2, col=1)
...
with t_dec:
    ...
    with cx2:
        ...
        fig_attr = px.bar(
            ...
            title="Relative Sensor & AI Modality Contribution to HI Penalty",
            text_auto=".1f"
        )
...
with t_whatif:
    ...
    col_w1.metric("Downtime Prevented", f"{whatif_res['savings']['downtime_saved_min']} min", delta="Reliability Gain")
    col_w2.metric("Financial Savings", f"${whatif_res['savings']['cost_saved_usd']}", delta="Cost Protection")
    col_w3.metric("OEE Efficiency Gain", f"+{whatif_res['savings']['oee_gain_pct']}%", delta="Productivity")
    col_w4.metric("Carbon Avoided", f"{whatif_res['savings']['carbon_saved_kg']} kg CO2", delta="Sustainability")
    ...
    wi_df = pd.DataFrame([
        {
            ...
            "Total Cost ($)": whatif_res["no_intervention"]["total_cost_usd"],
        ...
        },
        {
            ...
            "Total Cost ($)": whatif_res["predictive"]["total_cost_usd"],
            "Carbon (kg CO2)": whatif_res["predictive"]["carbon_kg"]
        }
    ])
    st.dataframe(wi_df.style.highlight_max(subset=["OEE (%)", "Good Units"], color="#d4edda").highlight_min(subset=["Downtime (min)", "Total Cost ($)", "Carbon (kg CO2)"], color="#d4edda"), use_container_width=True)
